...
src1 = cv2.imread('Resources\\no_coffee.png')
src2 = cv2.imread('Resources\\coffee.png')
diff_coffee = np.absolute(src2 - src1)

cv2.imshow('coffee diff image', diff_coffee)
cv2.waitKey(0)

############# 5b

_, thresh1 = cv2.threshold(diff_coffee, 127, 255, cv2.THRESH_BINARY)


cv2.imshow('binary filtered diff image', thresh1)
cv2.waitKey(0)

############# 5c

morph = cv2.morphologyEx(diff_coffee, cv2.MORPH_OPEN, kernel)
cv2.imshow('Morph filtered diff image', morph)
cv2.waitKey(0)

############# 5c


dest_xor = cv2.bitwise_xor(src1, src2, mask=None)
erod = cv2.erode(dest_xor, kernel)

cv2.imshow('Erod XOR filtered diff image', morph)
cv2.waitKey(0)
...
mask = np.where(diff_coffee < 230, diff_coffee, 255)
mask = np.where(mask > 230, diff_coffee, 0)
...
src1 = cv2.imread('Resources\\no_coffee.png')
new_image = np.where(mask < src1, src1, mask)
cv2.imshow('new mask', new_image)
cv2.waitKey(0)
